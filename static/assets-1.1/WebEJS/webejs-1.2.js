...
WebEJS_GEN.generate_model = {
  ...
  getModelCode : function(simulation, fullModel, forPackage) {
	...
	addElementsScriptCode : function(code, ejsModelElementsFolder, model) {
		...
		// if (fullmodel) 
    {
			...
			for (const jsLib of jsList) 
				code.push("    <script src='"+ejsModelElementsFolder+jsLib+"'></script>");
		}
   /*
		else {
			var lintList = [];
...
WebEJS_GEN.generate_ode = {
  ...
  oneODE : function(ode_page, page_counter, variable_pages) {
    ...
    for (const equation of ode_page['Equations']) {
      var state = equation['state'].trim();
      var rate = equation['rate'].trim();
...
WebEJS_GEN.generate_view = {
  ...
  getTreeCode : function(view_tree,description_pages,variables_list,fullmodel, forPackage) {
    ...
    if (view_tree.length)
      ...
      code.push(WebEJS_GEN.generate_view.__EMPTY_VIEW.replaceAll('{{WEBEJS_ASSETS_FOLDER}}',sMainAssetsURL));
 ...
 *  - useCDN : Use the provided CDN (versus using attached EJS library, as in a package)
...
WebEJS_GEN.generate = function(mOptions) {
  ...
  function __optionHasValue(keyword,value) {
   ...
   var mEjsAssetsFolder  = (__optionHasValue('useCDN', true) ? sMainAssetsURL : './');
   var mEjsLibraryFolder = mEjsAssetsFolder + sMainGUI.getSystemInformation('ejs_library_folder');
   var mEjsModelElementsFolder = mEjsAssetsFolder + sMainGUI.getSystemInformation('model_elements_folder');
   var mCodebaseFolder ;
  
  if (forPackage) mCodebaseFolder = null;
  else {
    const prefix = window.location.origin; //window.location.protocol + "//" + window.location.host;
  ...
  function __getMetadataCode(information) {
    ...
    for (const oneLogo of logos) {
      const logoURL = (WebEJS_TOOLS.isBase64Image(oneLogo) || !mCodebaseFolder) ? oneLogo : mCodebaseFolder + oneLogo;
      code.push("          <img alt='Logo' src='" + logoURL + "'/>");
    }
    code.push("        </p>");
    code.push("        <p>");
    for (const oneLogo of authorlogos) {
      const logoURL = (WebEJS_TOOLS.isBase64Image(oneLogo) || !mCodebaseFolder) ? oneLogo : mCodebaseFolder + oneLogo;
      code.push("          <img alt='author image' src='" + logoURL + "'/>");
    }
  ...
  function __getScriptsImportCode(simulation) {
    ...
    WebEJS_GEN.generate_model.addElementsScriptCode(code,mEjsModelElementsFolder,simulation['model']);
    return code.join("\n");
  }
...
WebEJS_GUI.comm = function(sessionID, mServerSubroute) {
	var self = {};
	const TEXT_TOOLS = WebEJS_TOOLS.textTools();

  console.log ("====== Server subrout = "+mServerSubroute)
	if (!mServerSubroute) mServerSubroute = '';
...
*/
	...
	function ajaxUpload(url, file, options, onSuccess, onErrorMessage, onErrorData, onErrorListener) {
...
//			headers: { "X-CSRFToken":csrftoken },
    ...
    $.ajax({
      ...
      success: function(blob, status, xhr) {
        ...
        var link = $('<a></a>');
        link.attr('href', url);
        link.attr('download', filename); // Nombre del archivo que se descargará
        window.setTimeout(function() { 
          //window.location = result['url'];
          link[0].click(); // Simula un clic en el enlace
          window.URL.revokeObjectURL(url); // Limpia el URL creado
        },500);
      },
      error: function(xhr, status, error) {
				ajaxError(error, 'Error downaloading file:'+filename, error);
      }
    });
  }
  ...
  // ----------------------------
    ...
    ajaxPost('/logout', { }, 
			() => { 
        console.log("WebEJS disconnected.") 
        window.location = "/";
      }, 
    ...
    ajaxPost('/system/system_information', {}, onSuccess, "Error reading system information from server!");
}

	// --------------------------------------------------------------
	// Common tools to load whatever is in the user's source folder
	// --------------------------------------------------------------
	
  function noSimulationFound() {
  ...
  function chooseSimulation(files) {
		const options = []
		for (i in files) options.push({'name' : files[i]});
    function selectListener (selected) {
      sMessageForm.showTemporal("Loading downloaded simulation...",
                                sLocaleFor("Cleaning files..."), 
                                "This message should dissapear automatically when done.");
      ajaxPost('/simulation/get', { 'file' : selected }, 
        response => { 
          if ('simulation' in response) sMainGUI.readResponse(response);
          else noSimulationFound();
        },"Error selecting file!"
      );
    }
    function cancelListener () {
      sMainGUI.logLine(sMainResources.getString("Reading default, empty simulation."));
      self.newSimulation();
    }
		sMainSelectChoiceForm.show('Choose simulation from the ZIP',options,files[0],
			selectListener,null,true,false,cancelListener);
	}

  var FIRST_TIME_RESPONSE = true;

	function processLoadResponse(response) {
    const no_files_warning = !FIRST_TIME_RESPONSE;
    FIRST_TIME_RESPONSE = false;
    ...
    if ('files' in response) {
      ...
      else if ('simulation' in response) return sMainGUI.readResponse(response);
		}
    else if ('simulation' in response) return sMainGUI.readResponse(response);
    else noSimulationFound();
	}
   ...
   * If there are several possibile choices, the user is asked to choose one.
   */
  self.getSimulation = function() {
    ajaxPost('/simulation/get', { }, 
			processLoadResponse, 
			"Error reading current simulation!");
	}

  /**
   * Updates the copy of the simulation in the server
   */
  self.updateSimulation = function() {
		var simulation = sMainGUI.saveObject();
    ajaxPost('/simulation/update', { 'simulation' : simulation }, 
			() => { console.log("Simulation updated to server.") }, 
  ...
  self.updateSimulationBeforeLeaving = function() {
    ...
    message = { 'simulation' : sMainGUI.saveObject(), 'session_id' : sessionID };
    postData('/simulation/update', message)
      .done(function(result) {
        // Aquí puedes usar el resultado de la llamada POST
        console.log('Simulation updated to session:'+ sessionID);
        // Realiza cualquier otra operación que necesite esperar la llamada POST
      })
   ...
   */
    ...
    ajaxPost('/simulation/new', { }, 
			response => { 
        sMainGUI.readResponse(response);
        sMainSimulationOptions.setAuthorInfo(sMainWebEJSOptions.getAuthorInfo());
      }, 
   ...
   */
	self.uploadUserZIPFile = function(file) {
		ajaxUpload('/simulation/upload', file, UNZIP_OPTIONS,
			response => {
	...
	// ----------------------------
	// Downloading from a library
	// ----------------------------

	function downloadLibraryModel (model_url,model_name) {
		sMessageForm.showTemporal("Downloading model...", model_name,
			"This message should dissapear automatically when done.");
    var options = UNZIP_OPTIONS;
    options['url'] = encodeURIComponent(model_url);
    ajaxPost('/simulation/download', options, 
      response => { 
        sMessageForm.hide();
        sMainGUI.logLine(sMainResources.getString("ZIP file correctly downloaded."));
        processLoadResponse(response);
      }, 
      "Error downloading library model: ",
      model_name
    );
  }  
	...
	self.openFromLibrary = function() {
		sMainLibraryChooser.show(
			function(model_url,model_name) { 
				downloadLibraryModel(model_url,model_name);
			}
		);
	}
    ...
    if (generate) {
      ...
      info['generated'] = WebEJS_GEN.generate(options);
    }
...
WebEJS_GUI.main = function() {
  ...
  self.getURLpathFor = function(path) { 
    if (WebEJS_TOOLS.isBase64Image(path)) return path;
    return mSimulationURLpath + path; 
  }

  self.getSystemInformation = function(keyword) { return mSystemInformation[keyword]; }
  self.getSystemIconURL = function(src) { return sMainAssetsURL+'WebEJS/icons/'+src; }
  self.getModelElementsURL = function() { return sMainAssetsURL+mSystemInformation['model_elements_folder']; }
  ...
  self.getView  = function() { return mViewPanel; }
  ...
  self.displayReadingWarnings = function(warnings) {
    if (warnings) {
      mReadingWarnings = [];
  ...
  self.addReadingWarnings = function(warnings) {
    for (var i=0; i<warnings.length; i++) {
      ...
      if (warning==('NEWLINE')) warning = '<br/>';
      else if (warning.startsWith('RAW:')) warning = '<span class="text-danger">'+ warning.substring(4)+ '</span> ';
      else if (warning.startsWith('RED:')) warning = '<span class="text-danger">'+ warning.substring(4)+ '</span> ';
      else warning = '<span class="text-primary">'+ sMainResources.getString(warning) + '</span> ';
  ...
  function doReadSimulation() {
    ...
    window.top.document.title = sMainVersionInfo; //+' - '+mSimulationFilename;

    if ('reading_messages' in mSimulationReceived)
      self.addReadingWarnings(mSimulationReceived['reading_messages']);
  ...
  self.readResponse = function(response) {
    ...
    mSimulationReceived = response['simulation'];
    mSimulationURLpath = response['url_path'];
    mReadingWarnings = [];
    if ('messages' in response) {
      self.addReadingWarnings(response['messages']); // But do not display until the read in complete
    }
...
`;
  ...
  function setLabelsLength() {
	...
	$('#mWebEJSOptionsEditorShorterLabels').on('change', setLabelsLength);
  ...
  $('#mWebEJSOptionsEditorAuthorIconField').change(function (event) {
    $('#mWebEJSOptionsEditorAuthorIconImage').attr("src", URL.createObjectURL(event.target.files[0]));
  });

  /*
  $('#mWebEJSOptionsEditorAuthorIconImage').on('load', function() {
    console.log('La imagen se ha cargado correctamente.');
    // Aquí puedes ejecutar cualquier otra función que desees
  }).on('error', function() {
    console.log('Hubo un error al cargar la imagen.');
  });
*/

  self.getAuthorInfo = function() {
    info = {};
    info['name']              = $('#mWebEJSOptionsEditorAuthorNameValue').val();
		info['affiliation']       = $('#mWebEJSOptionsEditorAuthorAffiliationValue').val();
		info['contact']           = $('#mWebEJSOptionsEditorAuthorContactValue').val();
    info['logo']              = WebEJS_TOOLS.getBase64("mWebEJSOptionsEditorAuthorIconImage");
		return info;
	}
  function checkDefaults(options) {
		if (!('use_short_labels' in options)) 				options['use_short_labels'] = "false";
		if (!('language' in options)) 				        options['language'] = "en";
		...
		if (!('author_logo' in options)) 					    options['author_logo'] = "";
  ...
  function readOptions(options) {
		...
		$('#mWebEJSOptionsEditorShorterLabels').prop("checked", (options['use_short_labels']=="true"));
    setLabelsLength();
    // Language is ignored, for the time being
    ...
    $('#mWebEJSOptionsEditorAuthorIconImage').attr("src", options['author_logo']);

    sMainGUI.getView().readUserCustomElements(options['user_elements']);
	}
  ...
  function collectValues(options) {
    ...
    options['author_logo']              = WebEJS_TOOLS.getBase64("mWebEJSOptionsEditorAuthorIconImage");

    options['user_elements'] = sMainGUI.getView().getUserCustomElements();
		return options;
	}

  $("#mWebEJSOptionsEditorImportButton").on ("click",(data)=>{
		$('#mWebEJSOptionsEditorField').click();
	});	
  
  function errorMessage(lineMessage) {
    mModal.hide();
    sMainGUI.errorLine(sMainResources.getString(lineMessage)); 
    sMessageForm.showWarning("Read error", "Error: Invalid WebEJS options file", 
      "Check out the output area.", () => { mModal.show(); });
  }

	$('#mWebEJSOptionsEditorField').change(function (event) {
    ...
    if (file && file.type === 'application/json') {
      ...
      reader.onload = function(e) {
        try {
          ...
          if ('__CONTENT__' in options && options['__CONTENT__']=='WEB_EJS_OPTIONS') readOptions(options);
          else errorMessage("The selected file is not a WebEJS options file");
        }
        catch(e) {
          console.log(e);
          errorMessage("The selected file is not a WebEJS options file");
        }
      };
      reader.readAsText(file);
    } 
    else errorMessage("The selected file is not a JSON file"); 
		$('#mWebEJSOptionsEditorField').val("");
	});

	$("#mWebEJSOptionsEditorExportButton").on ("click",(data)=>{
		const listener = function(name) {
      mModal.show();
			var filename = name.trim();
			if (filename.length <= 0) return;
      filename = TEXT_TOOLS.sanitizeFilename(name);

			const options = collectValues({ '__CONTENT__' : 'WEB_EJS_OPTIONS' });
    ...
    mModal.hide();
		sMainFilenameForm.show("WebEJS_options.json", listener);
	});
...
const sDescriptionIFrameOnLoad = function(hash) {
		  ...
		  const cssUrl = sMainAssetsURL+sMainGUI.getSystemInformation('ejs_library_folder')+'css/ejss.css'
		  var css = '<link href="'+cssUrl+'" rel="stylesheet" type="text/css" />';
...
WebEJS_GUI.descriptionWorkpanel = function(mMainPanelSelector) {
	...
	function getLocalePages(pages, locale) {
		...
		if (otherLocales) { 
			sMainGUI.addReadingWarnings([ 
        'The description of this simulation has non-default locale pages.',
				'RED:Locales (translations) are ignored and lost when you save!'
      ]);
		}
		return localePages;
	}
	...
	self.pageInitialization = function(mPageDiv, mPageObject) {
		...
		if (self.isPageSecondType(mPageDiv)) { // Initialization in case of external HTML
			...
			mPageDiv.find('.cFileChooser').on("click", (event)=>{ 
				...
				WebEJS_TOOLS.selectFile(WebEJS_TOOLS.HTML_FORMATS, 
...
WebEJS_GUI.elementsPanel = function() {
  ...
  var self = {};
	...
	self.buildPalette = function(systemInformation) {
		mMODEL_ELEMENTS_FOLDER = sMainGUI.getModelElementsURL();
...
WebEJS_GUI.optionsSimulationPanel = function() {
  ...
  self.setAuthorInfo = function(authorInfo) {
    if (authorInfo.name) {
      mAuthorsHash++;
      mAuthors.push({ 'name' : authorInfo.name, 'logo' : authorInfo.logo, 'hash' : mAuthorsHash });
      mOptions['Copyright'] = authorInfo.name;
      if (authorInfo.affiliation) mOptions['Copyright'] += ', '+authorInfo.affiliation;
      if (authorInfo.contact) mOptions['Abstract'] = 'Contact: '+authorInfo.contact;
      displayValues(mOptions);
    }
  ...
  self.setTitle = function(title) {
	...
	function displayValues(simInfo) {
		...
		{ // Authors and author logo
			...
			$('#mSimulationOptionsAuthorLogoGallery').on('click','img',function() {
				...
				WebEJS_TOOLS.selectFile(WebEJS_TOOLS.IMAGE_FORMATS, 
					function(path,urlPath) { 
      ...
      /*
			check = false;
			if ('UseIndexHtml' in simInfo) check = (simInfo['UseIndexHtml']=="true");
			$('#mSimulationOptionsUseIndexHtml').prop("checked", check);
      */
			...
			
		{
			...
			$('#mSimulationOptionsUserFilesAdd').click((event)=>{
				...
				WebEJS_TOOLS.selectFile(null, 
					...
					null, // No helper div
          true  // Allow selection of a folder
				);
			});
	...
	function collectValues() {
		...
		//mOptions['UseIndexHtml']    = $('#mSimulationOptionsUseIndexHtml').is(":checked") ? "true" : "false";
...
WebEJS_GUI.previewArea = function() {
	...
	self.clearPreview = function(title) {
		$('#sPreviewFrame').attr('src', sMainAssetsURL+'WebEJS/EmptyView/empty_view.html');
	...
	self.reportPreview = function(report) {
    updateLocally();
		//sMainComm.sendActionReport(report, onSuccessUpdate, onErrorUpdate);
	}
...
WebEJS_GUI.viewPropertyEditor = function(mViewPanel, mViewTree, mElementInfo) {
  ...
  const mutationCallback = (mutationsList) => {
    for (const mutation of mutationsList) {
      ...
      if (target.hasClass("adjust-done")) return;
      if (target.hasClass("show")) {
        console.log(target.attr('id') + " is now visible");
        adjustLabelSizesFor(target.attr('id'));
        target.addClass("adjust-done");
      }
    }
  }
...
*/
	{		
		const options = {
          ...
          mPanel.close(); 
				...
				setInteraction();
        /*
        $('.cPropertyEditorTabDiv').on('show', function(event) {
          console.log($( event.target ).id()+' is now visible');
        });
        $('.cPropertyEditorTabDiv').each(function() {
  ...
  function adjustLabelSizesFor(tabID) {
		$('#'+tabID+' .cViewPropertyEditorColumn').each(function(col, colDiv) {
			var maxWidth = 0; 
			$(colDiv).find('.cViewPropertyEditorLabel').each(function(index, label) {
				//console.log ("Width of label "+$(label).text() +' = '+$(label).width());
				maxWidth = Math.max(maxWidth,$(label).width());
	...
	function getContentHtml() {
		...
		for (var tabIndex in editionTabs) {
			...
			headerHtml += 
				...
				'</li>';
			divsHtml += 
				'<div class="tab-pane cPropertyEditorTabDiv fade'+activeDivTag+'" id="'+mId+'TabDiv_'+tabIndex+'"'+ 
...
WebEJS_GUI.viewPropertyTypeEditor = function(propertyTypeList, propertyModifiersList) {
  ...
  if (propertyTypeList.includes("Font")) return function (title,propertyName,value,onSuccess) { 
    ...
    if (propertyTypeList.includes("ImageFile")) supportedFormats.push(...WebEJS_TOOLS.IMAGE_FORMATS);
    if (propertyTypeList.includes("VideoFile")) supportedFormats.push(...WebEJS_TOOLS.VIDEO_FORMATS);
    if (propertyTypeList.includes("AudioFile")) supportedFormats.push(...WebEJS_TOOLS.AUDIO_FORMATS);

		WebEJS_TOOLS.selectFile(supportedFormats, 
			function(path,urlPath) { 
  ...
  $(mTreeViewHTMLElement).on('drop', '.sViewTreeElement', function (event) {
    ...
    if (info.startsWith('EJS-element')) moveOrReparent(hashOrType, treeItem);
			...
			if (elementType.startsWith("_custom_")) pasteElementList(treeItem,
				mViewPanel.getCustomElementList(elementType));
  ...
  function treeItemMenuAction(mMenuItem) {
    ...
    switch (action) {
      ...
      case "Remove" :
        ...
        break;
      case "AddCustom" :
        if (mViewPanel.isCustomSpaceAvailable()) {
          mCopiedList = [];
          recursivelyGetItem(mCopiedList, treeItem, null);
          sCustomElementForm.show(function(name,base64Image) { 
            mViewPanel.addUserCustomElement({ 'name' : name, 'icon' : base64Image, 'elements' : mCopiedList});
          });
        }
        break;
      case "Copy" : 
...
WebEJS_GUI.viewWorkpanel = function() {
  ...
  // These two care for all Custom Elements, both system and user defined
  var mCustomElementsList = {};
  var mCustomElementHashNumber = 0;

  // This is the list of user defined Custom Elements
  var mUserCustomElements = [];
...
//        '</img>'+
  ...
  function createNewCustomElementTag() {
    mCustomElementHashNumber++;
    return '_custom_'+mCustomElementHashNumber;
    //return '_custom_'+(isUserElement ? "user_" : "system_")+name;
  }

  self.getCustomElementList = function(elementTag) {
    return mCustomElementsList[elementTag];
  }
  
  self.getUserCustomElements = function() {
    return mUserCustomElements;
  }

  // ----- Adding custom elements 

  self.isCustomSpaceAvailable = function() {
    const emptyPlaces = $('#mPaletteGroup_Compound .cViewEmptyElement');
    if (emptyPlaces.length<1) {  
      const nUserEls = $('#mPaletteGroup_Compound .cViewUserElement').length;
      sMainGUI.errorLine(sMainResources.getString('You reached the maximum of custom elements')+':'+nUserEls); 
      sMessageForm.showWarning("Creation error", 'No space left', "Check out the output area.");
      return null;
    }
    return emptyPlaces;
  }

  self.addUserCustomElement = function(element) {
    const emptyPlaces = self.isCustomSpaceAvailable();
    if (!emptyPlaces) return;
    const html = createCustomElementHTML(element,true);
    $(emptyPlaces[0]).replaceWith(html);
    mUserCustomElements.push(element);
    return true;
  }

  function deleteUserCustomElement(elementTag) {
    const divs = $('#mPaletteGroup_Compound *[data-element_tag="'+elementTag+'"]');
    if (divs.length>0) $(divs[0]).replaceWith(createEmptyElementHTML ());
    delete mCustomElementsList[elementTag];
    for (var i=0; i<mUserCustomElements.length; i++) {
      const element = mUserCustomElements[i];
      if (element['tag']==elementTag) {
        mUserCustomElements.splice(i, 1);
        break;
      }
    }
  }
   ...
   * @param {*} userElements 
   */
  self.readUserCustomElements = function(userElements) {
    for (var i=0; i<userElements.length; i++) {
      const element = userElements[i];
      if (!self.addUserCustomElement(element)) break;
    }
  }
  ...
  function createCustomElementHTML (element, isUserElement) {
    var tag = createNewCustomElementTag();
    mCustomElementsList[tag] = element['elements'];
    if (isUserElement) element['tag'] = tag;
    const classname = isUserElement ? "cViewUserElement" :"cViewElementOnPalette";    
    return ''+
      '<div class="col px-1 py-1" style="user-select: none;" data-element_tag="'+tag+'">'+
        //'<img class="'+classname+'" src="data:image/gif;base64,'+element.icon+'" '+
        '<img class="'+classname+'" src="'+element.icon+'" '+
          ' draggable="true" ondragstart="WebEJS_GUI.viewElements_dragFunction(event,\'palette\')" data-drag_info="'+tag+'"' +
          ' data-element_type="'+element.name+'"' +
          ' data-bs-toggle="tooltip" data-bs-placement="top" title="'+element.name+'"'+
        '>'+
        '</img>'+
      '</div>';
  }
...
function createTabsHTML(panelCounter,tabs) {
    ...
    for (const [groupName, elementTypeList] of Object.entries(tabs)) {
      ...
      if (groupName=='Compound') {
        for (var index = 0; index < elementTypeList.length; index++) {
          ...
          contentHtml += createCustomElementHTML(compoundInfo,false); // Not a user element
          colCounter++;
  ...
  function addFileToList(list, filename) {
    ...
    if ( (filename.startsWith('"') && filename.endsWith('"')) || 
      ...
      list.push(filename.substring(1,filename.length-1));
      return;
    }
    if (filename.startsWith('[') && filename.endsWith(']')) { // It is an array
      const fileList = filename.substring(1,filename.length-1).trim().split(',');
      for (var i=0; i<fileList.length; i++) {
        addFileToList(list,fileList[i]);
      }
      return;
    }
  }
  ...
  self.getDetectedFiles = function() {
    const uniqueFiles = [...new Set(mViewHTMLTree.getDetectedPropertiesOfType('File'))]
    var detectedFiles = [];
    for (var i=0; i<uniqueFiles.length; i++) addFileToList(detectedFiles, uniqueFiles[i]);
    /*
      var file = uniqueFiles[i].trim();
      if (file.length<=0) continue;
      if (file.startsWith('/') || file.startsWith('\"/')) continue;
      if (file.startsWith('%')) continue;
      ...
      if (file.startsWith('[') && file.endsWith(']')) { // It is an array
        file = file.substring(1,file.length-1).trim();
        const fileList = file.split(',');
        for (var j=0; j<fileList.length; j++) {
          var eachFile = fileList[i].trim();
          if (eachFile.startsWith('"') || eachFile.startsWith["'"]) detectedFiles.push(file);
      }			
    }
    */
    return [...new Set(detectedFiles)];
  }
  ...
  */
  ...
  $('#sViewPaletteSubpanel').on('dblclick', '.cViewUserElement', function (event) {
    sMainConfirmationForm.show($(event.currentTarget).data("element_type"),
      ...
      "Delete",
      function() { 
        const tag = $(event.currentTarget).data("drag_info");
        console.log ("tag = "+tag );
...
WebEJS_GUI.filenameForm = function(mTranslator) {
	...
	var MODAL_HTML = WebEJS_GUI.MODAL_TEMPLATE
		...
		.replace( /#\{TITLE\}/g,  'Choose filename' )
...
`;
  ...
  var mCancelListener;
	...
	self.show = function(title, options, currentValue, listener, 
    modalClass, disableInput, includeIndexForArrays,
    cancelListener)  {
		mListener = listener;
	...
	$('#mSelectChoiceFormCloseButton').click((event)=>{
    if (mCancelListener) mCancelListener();
	});		
	return self;
}
...
WebEJS_TOOLS.IMAGE_FORMATS = ['jpeg', 'jpg', 'gif', 'png', 'svg'];
WebEJS_TOOLS.VIDEO_FORMATS = ['mp4', 'ogv', 'webm'];
WebEJS_TOOLS.AUDIO_FORMATS = ['aac', 'mp3', 'm4a', 'ogg', 'wav'];
WebEJS_TOOLS.HTML_FORMATS  = ['html','xhtml'];
...
WebEJS_TOOLS.isBase64Image = function(imageStr) {
  const base64ImagePattern = /^data:image\/(png|jpg|jpeg|gif|bmp|webp);base64,/i;
  return base64ImagePattern.test(imageStr);
}
...
WebEJS_TOOLS.getBase64 = function(imageElementID) {
  ...
  WebEJS_TOOLS.selectFile = function(extensions, onSuccess, checkExistence, helperHtml, foldersToo) {
	...
	sMainFileChooser.showRead(extensions, fileListener, helperHtml, foldersToo);
}
	/*
 * Copyright (C) 2021 Jesús Chacón, Francisco Esquembre and Félix J. Garcia 
 * This code is part of the Web EJS authoring and simulation tool
...
WebEJS_TOOLS.textTools = function() {
  ...
  self.isValidFilename = function(filename) {
    // Define a regex pattern for valid characters: alphanumeric, hyphens, underscores, and dots
  ...
  self.sanitizeFilename = function(filename) {
    ...
    filename = filename.split('').map(char => accentsMap[char] || char).join('');
  
    // Reemplaza los caracteres no alfanuméricos (excepto guiones, guiones bajos y puntos) por guiones bajos
    filename = filename.replace(/[^a-zA-Z0-9._-]/g, '_');
  
    return filename;
  }
  
  self.toValidVariableName = function(str) { 
    // Remove invalid characters and replace them with underscores
    var validName = str.replace(/[^a-zA-Z0-9_$]/g, '_');
    
    // Ensure it doesn't start with a number
    if (/^[0-9]/.test(validName)) validName = 'notThis_'+validName;
    
    // Avoid reserved keywords (example check for some common keywords)
    const reservedKeywords = ["break", "case", "catch", "class", "const", "continue", "debugger", "default", "delete", "do", "else", "enum", "export", "extends", "false", "finally", "for", "function", "if", "import", "in", "instanceof", "new", "null", "return", "super", "switch", "this", "throw", "true", "try", "typeof", "var", "void", "while", "with", "yield"];
    
    if (reservedKeywords.includes(validName)) {
        validName = 'notThis_'+validName;
    }
